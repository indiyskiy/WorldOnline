...
<head>
    ...
    <title>Добавить тег в карточку</title>
...
<header>
    ...
    <div class="container container-lower">
        <form action="addtagtocard" method="post">
            <c:forEach items="${tagGroups}" var="tagGroup">
                <c:if test="${not tagGroup.isEmpty}">
                    <dl class="spoiler">
                        <dt onclick="clickSpoiler(this);">${tagGroup.name}</dt>
                        <dd>
                            <c:forEach items="${tagGroup.tagList}" var="tag">
                                <input type="checkbox" name="tagID" value="${tag.tagID}"
                                        <c:if test="${tag.added}"> checked</c:if>/>
                                ${tag.tagName}
                                <br>
                            </c:forEach>
                        </dd>
